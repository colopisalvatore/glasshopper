 ...
 *
 * Persistence is two-layer: `localStorage` is a synchronous write-through cache
 * (instant first paint, offline resilience), and Home Assistant storage —
 * reached over the websocket bridge ({@link loadRemote}/{@link saveRemote}) —
 * is the synced source of truth, so a mapping follows the dashboard across
 * browsers. When the integration is too old to answer (or there's no
 * connection), the remote calls degrade silently to the cache.
 */
import { sendCommand } from '@/lib/haConnection';
 ...
 */
export function dashboardSlug(): string {
  try {
    const path =
...
/* ------------------------------------------------------------------ */
/*  Home Assistant storage — synced source of truth (best-effort).     */
/* ------------------------------------------------------------------ */

/** Fetch this dashboard's mapping from HA, or null when unavailable/unset. */
export async function loadRemote(slug: string): Promise<StoredConfig | null> {
  try {
    const res = await sendCommand<{ config: { map?: GhConfig; seen?: boolean } | null }>(
      'glasshopper/config/get',
      { slug },
    );
    const c = res?.config;
    if (c && c.map && typeof c.map === 'object') {
      return { v: 1, map: c.map, seen: Boolean(c.seen) };
    }
    return null;
  } catch {
    // Integration too old to know this command, or no connection — caller keeps
    // the local cache.
    return null;
  }
}

/** Persist this dashboard's mapping to HA. Requires admin server-side. */
export async function saveRemote(slug: string, stored: StoredConfig): Promise<void> {
  try {
    await sendCommand('glasshopper/config/set', {
      slug,
      map: stored.map,
      seen: stored.seen,
    });
